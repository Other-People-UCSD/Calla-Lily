...
Instead of using the recommended TinaMarkdown component for parsing markdown, I use React’s dangerouslySetInnerHtml prop to inject the raw text into the page. However, there are key-value differences inside the Abstract Syntax Tree (AST) where html and html\_inline nodes have the key ‘value’ to denote the content string and regular markdown nodes have the key ‘text’ for the content string.

Currently, the custom parsing function only checks the content one level down the rich text AST and appropriately parses the HTML in markdown correctly.
