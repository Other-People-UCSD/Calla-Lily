...
[You Have Created An Imaginary Friend](/6/you-have-created-an-imaginary-friend)

[cyoa.js (source code)](https://github.com/Other-People-UCSD/Calla-Lily/blob/main/public/js/cyoa.js)

["You Have Created An Imaginary Friend" JSON data](https://github.com/Other-People-UCSD/Calla-Lily/blob/main/public/js/imaginary-friend-theo-erickson.json)

## Abstract

The choose-your-own-adventure (CYOA) program is meant to give the end user a more understandable experience of the
story by allowing them to know their progress through the story. This documentation goes through how the CYOA workflow
using Theo Erickson’s “You Have Created An Imaginary Friend” story as a reference.

## Context

Usually a CYOA story may be drafted as a Twine or other dialogue story file. Twine has an undo and redo feature for
passages, but lacks any progress indicator for the user to see just how long the story is. This lack of indicator can
also confuse users who think there isn’t more to the story if they get lost and keep repeating routes. In addition,
publishing a Twine file can take the form as a third-party website that we cannot manage or a standalone HTML file. That
HTML file that does not work well with the Jekyll-based and React-based website’s ability to template an HTML page. For
an enhanced UX and UI experience that can natively run on the Other People Website via DOM injection, our own CYOA
program is used.
...
It can be difficult for staff editors to edit the story off of Twine when the Twine file does not sync state and can
only be imported by a .twine file. Instead, CYOA stories should be written in a collaborative text editor such as Google
Docs where all viewers will see the same version. To organize the story, each passage should be on their own page.
Adding a header such as the route or id number allows for internal hyperlinks to work where routes from other passages
can link to and makes stepping through the story arc feel very similar to Twine or the CYOA program.
...
Script to extract the data, especially if a passage extends more than one page. The downside of using a spreadsheet is
that it is harder to review the raw text as compared to a text editor, so this is something that the website developer
...
read data that has hundreds of unicode encodings for commas scattered in every passage.
...
raw text or HTML. If a passage’s text is raw and there are multiple paragraphs, the data has to understand there are
multiple paragraphs that cannot be identified by a space since a story will have spaces throughout it. Double spaces are
rarer and can be unicode encoded as well. Another option is to just change the delimiter used to something else.
...
easier to work with data locally through a notebook instead of waiting for an asynchronous HTTP request to get the data
...
![A directed graph in tree form of the routes.](/images/docs/imaginary-friend-graph-tree-back.png)
...
to be made. **The “You Have Created An Imaginary Friend” JSON file is 85.1 KB in size.**
...
webpage from either navigating away or hard refreshing the page will reset any route history the reader has had before.
...
<p class="center"><img src="/images/docs/cyoa-bubbling-state.png" alt="Diagram showing bubbling coloration of passages up a route leaf." class="w450" /></p>

The progress bar updates when an unvisited route is clicked on. There has been reported cases where it can go one over
...
<p class="center"><img src="/images/docs/imaginary-friend-create-story.png" alt="Screenshot showing the passages a user can create a story." class="w650" /></p>
...
back and reset button and progress bar should be displayed for each story that uses the program.

Another diagram of “You Have Created An Imaginary Friend” without the edges back to start (node 1).

![A directed graph in tree form of the routes.](/images/docs/imaginary-friend-graph-tree-forward.png)
