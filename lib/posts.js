...
export async function getPostDataAPI(slug, postsData) {
  const fPath = buildFilePath(slug);
...
export async function getPostDataAPI2(relativePath, postsData) {
  ...
  // console.log('id', id)
  // console.log('keyedPosts', keyedPosts);
  return {
    wordCount,
    contentHtml,
    ...keyedPosts[id],
...
export async function getPostDataAPI3(relativePath, postsData) {
  const fullPath = path.join(postsDirectory, relativePath);
  const fileContents = fs.readFileSync(fullPath, 'utf8');
  const matterResult = matter(fileContents);
  const processedContent = await remark()
