...
export function getSortedPostsData() {
  ...
  for (let i = allSorted.length - 1; i >= 0; i--) {
    ...
    if (prev) {
      allSorted[i].prevPost = `${prev.collection}/${prev.slug}`;
    }
    const next = allSorted[i - 1] || null;
    if (next) {
      allSorted[i].nextPost = `${next.collection}/${next.slug}`;
    }
  }
...
export function getAllPostIds() {
  ...
  return fileNames.map((fileName) => {
    const { collection, slug } = reformatFileId(fileName);
    return {
      params: {params: [collection, slug]}
 ...
 */
export async function getPostData(slug, collection) {
  const fPath = buildFilePath(slug, collection);
...
export async function getPostDataAPI(params, postsData) {
  console.log('api', params.params);
  const fPath = buildFilePath(params.params[1], params.params[0]);
  ...
  const wordCount = matterResult.content.split(' ').length;
  const contentHtml = processedContent.toString();
  const id = `${params.params[0]}/${params.params[1]}`;
...
export function reformatFileId(fileName) {
  ...
  const collection = fileName.substring(0, fileName.indexOf('\\'));
  const slug = fileName.substring(fileName.indexOf('\\') + 1).replace(/\.md$/, '');
 ...
 * @returns 
 */
export function buildFilePath(slug, collection) {
