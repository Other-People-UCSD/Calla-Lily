...
/**
 * USED FOR [...slug].json and /api/post-metadata.json
 * Gets all the posts in the postsDirectory sorted by date
 * @returns {Object} Posts sorted by most recent date
 */
 ...
 * @returns {String}  The collection folder and the file's title without the .md ext
 */
 ...
 * @returns {Array} All files in the target directory 
 */
...
/**
 * USED FOR /api/pages.json
 * For auto-generating the valid pages in the sitemap.
 * @returns {Promise<Object>} The valid landing pages
 */
export async function getLandingPages() {
  return new Promise((resolve, reject) => {
    ...
    if (fileNames.length === 0) {
      reject(new Error("No pages were found."));
    }
    ...
    const postFileNames = getAllFiles(postsDirectory);
    const allFiles = getAllFiles(path.join(process.cwd()));
    resolve({
      fileNames,
      pages,
      validPages,
 ...
 * @returns {Object} Returns post with all content information
 */
